...
- `scikit-learn` is pinned to `==1.6.1` in `requirements.txt`, the version `scaler.pkl` was pickled with. Don't unpin it: the scaler is loaded as-is and never refit, so a `MinMaxScaler` internals change would shift every prediction with only an `InconsistentVersionWarning` to show for it. Re-fitting under a newer version needs `train_FD001.txt`, which isn't in the repo. An environment installed before this pin may still be on a newer version — check with `python -c "import sklearn; print(sklearn.__version__)"` if you see that warning at startup.
